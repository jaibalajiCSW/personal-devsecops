...
def process_config(config):
  try:
    ...
    auth_token = "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"
  ...
  return platform_url, api_key, client_id, network_id, git_user, repo_name, auth_token, zipfile_name
...
def extract_zip(git_user,repo_name,artifact_id,auth_token,zipfile_name):
        ...
        try:
          with ZipFile(file_name, 'r') as zip:
            ...
            zip.extract(member="depend.csv", path=".")
        except: 
...
def main():
  ...
  platform_url, api_key, client_id,network_id, git_user, repo_name, auth_token, zipfile_name = process_config(config)
  #CHECKING FOR MISSING VARIABLES
  if (git_user == "" or repo_name == "" or auth_token == "" or zipfile_name == "" ):
  ...
  upload_file(upload_id,platform_url,client_id,api_key,File_name)
